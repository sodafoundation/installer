...
while getopts ":i:u:r:" action; do
    ...
    i)  PACKAGES="${OPTARG}"
        ...
        linux*)  if [[ -f "/usr/bin/apt-get" ]]; then
                     ...
                     /usr/bin/zypper --non-interactive install -y "${PACKAGES}" || exit 1
    ...
    u)  PACKAGES="${OPTARG}"
        ...
        linux*)  if [[ -f "/usr/bin/apt-get" ]]; then
                     ...
                     /usr/bin/zypper --non-interactive up -y "${PACKAGES}" || exit 1
                 elif [ -f "/usr/bin/yum" ]; then
    ...
    r)  PACKAGES="${OPTARG}"
        ...
        linux*)  if [[ -f "/usr/bin/apt-get" ]]; then
                     ...
                     /usr/bin/zypper --non-interactive rm -y "${PACKAGES}" || exit 1
                 elif [ -f "/usr/bin/yum" ]; then
